...
1、更改了一些默认设置试图避免辣鸡电脑可能出现的一些已知问题

2、去除默认限速为10m/s的限制，但这可能会导致出现问题，可以手动设置

3、新增了日志生成功能，自动保存在log文件夹里的log.out文件中，而不是显示在窗口上
...
解：端口被占用了，最简单的方法是重启电脑。在此之前请先检查hosts文件，不要有pandownload.com
...
解：等待10秒内还卡在这的话请更换下载接口，部分电脑需要重新启动程序
...
解：在百度云网页版分享，若遇到“该文件禁止分享”，可以新建文件夹把文件移动进来，再移动一些别的文件进来，然后连文件夹一起分享
...
答：我们不仅去除了亿寻的区域限制、单日下载流量限制，而且保留了原pandownload的大多数很好用的功能，比如资源搜索、批量重命名、文件清理、一键取消分享、自动获取提取码等

18、为什么保留了默认共享提取码设置？

答：为了反抗资本主义，建设自由和平开放共享的和谐社会，不过您可以手动关闭（设置-功能-分享链接共享-关闭）
